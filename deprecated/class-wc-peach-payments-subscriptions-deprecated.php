...
class WC_Peach_Payments_Subscriptions_Deprecated extends WC_Peach_Payments_Subscriptions {
	...
	function scheduled_subscription_payment( $amount_to_charge, $order, $product_id ) {
		...
		if ( is_wp_error( $result ) ) {
			$order->add_order_note( __('Scheduled Subscription Payment Failed: Couldn\'t connect to gateway server - Peach Payments', 'woocommerce-gateway-advanced-peach-payments') );
			WC_Subscriptions_Manager::process_subscription_payment_failure_on_order( $order, $product_id );
		} elseif ( $result['PROCESSING.RESULT'] == 'NOK' ) {
			$order->add_order_note( sprintf(__('Scheduled Subscription Payment Failed: Payment Response is "%s" - Peach Payments.', 'woocommerce-gateway-advanced-peach-payments'), woocommerce_clean($result['PROCESSING.RETURN']) ) );
			WC_Subscriptions_Manager::process_subscription_payment_failure_on_order( $order, $product_id );
		} elseif ( $result['PROCESSING.RESULT'] == 'ACK' ) {
			$order->add_order_note( sprintf(__('Scheduled Subscription Payment Accepted: Payment Response is "%s" - Peach Payments.', 'woocommerce-gateway-advanced-peach-payments'), woocommerce_clean($result['PROCESSING.RETURN']) )  );
			WC_Subscriptions_Manager::process_subscription_payments_on_order( $order );
		}else{
			$order->add_order_note( __('Scheduled Subscription Payment Failed: An unknown error has occured - Peach Payments', 'woocommerce-gateway-advanced-peach-payments') );
			WC_Subscriptions_Manager::process_subscription_payment_failure_on_order( $order, $product_id );
		}
	...
	function execute_post_subscription_payment_request( $order, $amount, $payment_method_id ) {
		...
		$subscription_name = sprintf( __( 'Subscription for "%s"', 'woocommerce-gateway-advanced-peach-payments' ), $product->get_title() ) . ' ' . sprintf( __( '(Order %s)', 'woocommerce-gateway-advanced-peach-payments' ), $order->get_order_number() );
		...
		if ( is_wp_error($response) )
			return new WP_Error( 'peach_error', __('There was a problem connecting to the payment gateway.', 'woocommerce-gateway-advanced-peach-payments') );
	
		if( empty($response['body']) )
			return new WP_Error( 'peach_error', __('Empty response.', 'woocommerce-gateway-advanced-peach-payments') );
		...
		try {
			...
			if ( isset( $_POST['peach_payment_id'] ) && ctype_digit( $_POST['peach_payment_id'] ) ) {
				...
				if ( ! isset( $payment_ids[ $_POST['peach_payment_id'] ]['payment_id'] ) ) {
					throw new Exception( __( 'Invalid', 'woocommerce-gateway-advanced-peach-payments' ) );
				} else {
			...
			wc_add_notice( __('Error:', 'woocommerce-gateway-advanced-peach-payments') . ' "' . $e->getMessage() . '"' , 'error' );
			return;
		}
	}	
